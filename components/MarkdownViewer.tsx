...
export default function MarkdownViewer({ content = "# Beispiel Markdown\n\nDies ist ein Beispieltext für das Schreibtraining." }: MarkdownViewerProps) {
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (nextRequiredChar === '\n' && event.key === 'Enter') {
            ...
            setLastInput('↵')
        } else if (event.key === nextRequiredChar) {
    ...
    useEffect(() => {
        if (missingCharRef.current) {
            const rect = missingCharRef.current.getBoundingClientRect();
            console.log(`Missing char position - Height: ${rect.top}, Width: ${rect.left}`);
        }
    }, [missingCharRef.current]);
            ...
            <Card className="flex-grow overflow-hidden">
                <CardContent className="p-6 h-full">
                    ...
                    <input
                        ...
                        className="absolute opacity-0 top-0 left-0 w-full h-full cursor-default pointer-events-none"
                        autoFocus
                    />
                </CardContent>
            </Card>
